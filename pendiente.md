...
| A4 | **Contenido sensible: lo elige el colegio al contratar.** Decidido el 31/08: la lista (sexualidad de 7°, pubertad y sistema reproductor de 6°, drogas y alcohol de 6° y 4°, conquista y Araucanía en Historia) **no se conversa antes** — se ofrece como opción al momento de contratar. ⚠️ **Eso implica una feature que hoy NO existe:** el armador solo *marca* lo sensible para enlaces de muestra (A7); apagar un OA de verdad toca la campaña, el **Jefe Final** —que mezcla objetivos de todo el capítulo— y el mapa de dominio del profesor. Mientras no exista, la única forma real de excluirlo es no incluir ese capítulo | ~1 sesión | código, cuando se venda |
...
| **A25** | ⏳ **EN CURSO (02/09).** ✅ **Fases 1 a 4 hechas: el motor está en `assets/js/lecciones.js` y las 45 mini-clases están escritas y jugadas** (7°: 19 · 3°: 26 · 8°: las 17 de siempre = **62**). Los tres forks bajan a **8.301 líneas** (−4.157). El catálogo de dibujos pasó de 12 a **22**: 3 para 7° (`circulo`, `poligono`, `figura`) y **5 para 3°** (`dinero`, `cuadricula`, `reloj`, `pictograma`, `puntos`) — el plan preveía 2. La mini-clase estrena **🔊 en 3°**, que era la pantalla con más texto del juego y la única sin voz. ⚠️ **Dos bugs reales de motor encontrados jugando:** la práctica sacaba el banco del **nombre de la asignatura**, y como 3° escribe `'Matemática'` en singular caía al banco de **8°**, no encontraba ningún `MA03` y **marcaba la lección completa sin medir nada, sin ningún error** (quinto caso del patrón del fork; ahora sale de `CFG.ruta`); y el botón «← Salir» se comía la barra de progreso entera (**406 px contra 0**), defecto **vivo en producción en 8°**, ahora 96/205. **Falta:** Tarea 14 (aprobación de las 45, es de Roberto) · Tarea 15 (voz de 3°, ~230 clips ≈ US$0,15, **después de aprobar y con autorización**) · Fase 5 (las 13 introducciones de Ciencias). Detalle del formato en [`docs/estandar-miniclases.md`](docs/estandar-miniclases.md). **Lo que sigue vigente del encargo original:** Salió del encargo de A24. **Alcance decidido: 45 mini-clases** (7°: 19, 3°: 26) con el mapa «enseña → desafío» de 8°, más **13 introducciones de Ciencias** (una por capítulo). ⚠️ **4°, 5° y 6° NO entran**: la práctica saca del banco (`fromBank`) y sus bancos no existen — dependencia dura con el Bloque B, sus 78 quedan planificadas. **Tres cosas que hay que saber antes de empezar:** (1) ⚠️ **ninguna herramienta lee `lecciones.json`**, así que las 17 de 8° llevan **enseñando sin firma desde la Sesión 29** y la aprobación va **antes** de escribir; (2) el motor vive **solo en 8°** y hay que extraerlo a `assets/js/lecciones.js` (479 líneas + 4 funciones sueltas por balance de llaves), y ⚠️ **`motor.js` nombra tres de ellas** —`renderCampañaMate`, `volverAlCapituloMate`, `abrirMiniClaseDeOA`—, así que cortarlas sin reconectar mata los tres cursos; (3) las introducciones de Ciencias **no piden una línea de motor**, porque `terminarLeccion()` ya marca completa una lección sin práctica. Voz de 3°: ~230 clips ≈ **US$0,15**, al final y con autorización. Plan de **18 tareas en 5 fases**: [`docs/superpowers/plans/2026-09-02-miniclases-e-introducciones.md`](docs/superpowers/plans/2026-09-02-miniclases-e-introducciones.md) | ~3-4 sesiones | código |
