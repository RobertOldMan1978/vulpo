...
## Dónde estamos hoy (06/09/2026, medido en disco)
...
| 8° | 80 | 2.644 (+482 apoyo) | ✅ todas | no lleva | ✅ portadas 20/20 | ✅ | ✅ |

- **16.995 preguntas escritas y 233 lecciones · todas aprobadas (24/09).** Las últimas 130 son el
  **segundo libro de 8°** (*Teatro escolar representable 1*, la antología de Rubén Unda; 11 obras,
  escrito el 23/09 y firmado el 24/09). ⚠️ **El 100% es una foto, no un estado, y aquí está la
...
- **VULPO se anuncia desde 3° a 8° básico (06/09/2026), y desde ese mismo día los SEIS cursos
  ...
  7°, 8°) con los números reales: **16.995 preguntas aprobadas, 529 objetivos medidos** y 6 cursos
  completos. ⚠️ Los objetivos **no** suben con el Vocabulario ni con los libros: sus códigos
  `VOC-*` y `TE-T#`/`AF-T#`/`CA-T#` no son OA del currículum, así que suman preguntas y no
  cobertura medida.
- **Sitio publicado: 742 MB** (techo de GitHub Pages: 1 GB), remedido el 07/09 tras generar la voz
  de 4°. `assets/` completo son 840 MB: 602 MB de voz (3° y 4°, 6+4 asignaturas — 4° sumó 326 MB,
...
- **Backend al día (07/09):** los códigos de los seis cursos en las dos listas de
  `kimun_prof_asignaturas`, la foto semanal agendada y **tomando fotos desde el 31/08**, y
  **`kimun_prof_tendencia` aplicada y comprobada** —responde `no_autorizado` a un anónimo,
  mientras una función inventada da `PGRST202`: sin ese control negativo el error no probaría
...
- **Paridad de funcionalidad entre los seis forks: completa donde corresponde.** Los seis
  ...
  cuenta el tablero. Ese día Historia pasó de 12 a **33** —todos sus capítulos llevan
  introducción— y Matemática de 89 a **140**: el camino enseña→desafío llegó a los seis cursos:

  | | 3° | 4° | 5° | 6° | 7° | 8° |
  |---|---|---|---|---|---|---|
  | Mini-clases de Matemática | 26 | 27 | 27 | 24 | 19 | 17 |
  | Lecciones de Lenguaje | 6 | 6 | 5 | 5 | 6 | 5 |
  | Introducciones de Ciencias | 4 | 5 | 4 | 5 | 5 | 4 |
  | Introducciones de Historia | 5 | 5 | 6 | 6 | 6 | 5 |
  | Reto Sin Fin · Reto de Cálculo | ✅ · — | — · — | ✅ · — | — · — | ✅ · — | — · ✅ |

  Cifras **contadas de los `lecciones.json`** el 07/09, no copiadas: 140 + 33 + 27 + 33 = 233.
  **4° y 6° recibieron sus 51 mini-clases de Matemática el 07/09** (Sesión 104), así que ya no
  quedan cursos con `HAY_MINICLASES=true` sin camino enseña→desafío.
...
| **A58** | ✅ **HECHA (23 y 24/09, Sesiones 132-133).** **Segundo libro de 8°**: *Teatro escolar representable 1*, la antología de teatro de Rubén Unda (Zig-Zag), en `contenido/lectura-teatro-escolar-1`. **11 tramos —uno por obra— y 130 preguntas**, escritas **desde el ejemplar** (a diferencia de *Cuentos de Ada*, que salió de resúmenes, así que este sí admite detalle). 8° pasa a ser el primer curso con **dos** libros, y no costó una línea de motor: la biblioteca ya recorría un catálogo. **Roberto las firmó el 24/09** y el banco volvió a 16.995 de 16.995; el diff fueron **131 líneas** (las 130 marcas y su contador), sin reformateo, y el tablero se verificó **con control positivo** —desmarcar una a propósito y comprobar que la detecta—, que es lo único que distingue un banco completo de un detector ciego. ⚠️ **Siguen faltando las páginas 10-12 del escaneo** (el desenlace de la primera obra): ese tramo lleva **7 preguntas y ninguna toca el final**, y se completa si Roberto las pasa → **A59** | hecha | Roberto |
| **A59** | **Completar la primera obra del libro de teatro de 8°** (opcional). Al escaneo que pasó Roberto le **faltan las páginas 10 a 12** —el desenlace de *Castigo y recompensa*—: el PDF salta de la 9 a la 13. Por eso ese tramo lleva **7 preguntas y ninguna toca el final**, ni el «castigo» ni la «recompensa» del título, y así quedó declarado en su `nota_fidelidad` — **menos preguntas antes que inventar un desenlace**, que es lo que protege al alumno que sí leyó. ⚠️ Y en la misma obra hay una **errata del propio libro**: su lista de PERSONAJES **no nombra a la Abeja**, que el narrador sí incluye entre los cuatro criados; sin esas páginas no se puede zanjar, así que **ninguna pregunta se apoya en esa lista**. Con las tres páginas se escriben las que faltan (~5) y se vuelven a firmar | ~30 min | Roberto (pasar las páginas) |
...
| **A29** | ✅ **HECHA (06/09, Sesión 98).** Lo vio Roberto en una captura de Matemática de 3°: **la campaña repetía el arte**. Medido, no era parecido sino **el mismo archivo dos veces seguidas** — `renderCampañaMate` pintaba por unidad la tarjeta de la mini-clase y la de su "Expedición · X", y **las dos llamaban a `portadaUnidad(cap,c)`**, en las cuatro campañas con mini-clases (3°, 5°, 7°, 8°). Roberto pidió el arreglo de fondo y no el cosmético: *"las lecciones deben ser lo primero de cada expedición… y eso debe ser igual con las introducciones"*. Ahora hay **una sola tarjeta por unidad**, que abre directo su expedición, y las mini-clases se dibujan como **primeros nodos DENTRO de ese mapa** (`LECC.nodoLecciones`), igual que una introducción de Ciencias — **con la diferencia de que estas SÍ bloquean**: es "aprender desbloquea el desafío" desde la Sesión 29. ⚠️ **El bloqueo es de pantalla y de clic, no de datos**: `renderMapa` muestra la etapa 0 cerrada mientras falte una lección, **sin tocar `progAct()[0]`**, así que ningún save viejo se corrompe. Todo vive en los **dos módulos compartidos** (ni un fork tocado, salvo el respaldo `nodoLecciones` idéntico en los seis). ✅ **Y de paso, el 🔊 de la mini-clase estrena resaltado tipo karaoke**: `leerSegmentosEnVoz` (en `voz.js`) ilumina el fragmento que va sonando, repartido **por proporción de caracteres sobre la duración real del audio** — porque el generador junta la intro y los pasos con `". "` y hay **un solo clip por bloque**, así que un resaltado exacto por palabra habría exigido regenerar toda la voz (opción descartada por Roberto). Sin clip cae a `speechSynthesis`, que da `onboundary` real. Verificado jugando en los cuatro cursos: una tarjeta por unidad, etapa 1 bloqueada hasta terminar las lecciones y abierta al terminarlas, el karaoke recorriendo los 6 fragmentos de "Contar de a saltos" en un clip real de 20,1 s, y cero consola / cero 404. ⚠️ **Y lo vio Roberto: el arreglo servía en la campaña y NO en los enlaces de prueba.** Qué lecciones le tocan a una expedición se resolvía en el **clic de la tarjeta de la campaña**, y **a una expedición se entra por CUATRO caminos** (campaña, `?solo=`, `?m=`, armador): en tres de ellos ese clic nunca ocurre, así que un profesor con enlace de muestra abría **el quiz sin una sola mini-clase adelante**. Es el patrón de la Sesión 42 —el modo prueba nunca pasa por la pantalla de campaña— y se corrigió moviéndolo a `activarExpedicion` (`leccionesDeExpedicion`, que **deriva la unidad desde la campaña** en vez de recibirla del punto de entrada; el estado vive en `LECS_EXP` y ya no muta el objeto de datos). Verificado en los cuatro caminos. ✅ **Y por decisión de Roberto, las unidades de mini-clases salieron de `EXTRAS`**: con la expedición conteniéndolas, la casilla suelta le ofrecía al profesor la misma unidad dos veces. Quitadas de los cuatro cursos (7+6+4+4); `EXTRAS` queda solo con el Reto de Cálculo y el Reto Sin Fin, y **en 5° queda vacío**. Con eso `LECC.abrirUnidad` quedó sin llamador y se borró, con su respaldo en los seis forks. ⚠️ **Consecuencia asumida:** un enlace repartido con el id de una unidad (`?solo=mate-numeros`) deja de ser válido y **cae al juego normal** (comprobado: `SOLO` vacío, sin reventar) — el equivalente de hoy es su expedición, que trae clase y quiz juntos | hecha | código |
| **A31** | ✅ **HECHA (06/09, Sesión 98).** ⚠️ **La ✕ del quiz llevaba días muerta en 3°, 5° y 7°, en producción.** Lo encontró Roberto usándola: en la práctica de una mini-clase no había cómo volver atrás. El handler llama `volverAlCapituloMate()`, que **vive dentro del IIFE de `lecciones.js`** desde que la Sesión 83 sacó el motor a un módulo: **solo 8° se migró** a `LECC.volverAlCapitulo()`, 3° y 7° quedaron sin migrar y 5°, 6° y 4° nacieron copiando de ahí. ⚠️ **Y falla MUDO**: un `ReferenceError` dentro de un `onclick` no llega a la consola de forma fiable, así que el botón no hace nada y todo lo demás se ve bien — misma familia que el JUGADOR de 7° (Sesión 90). **La instrucción estaba escrita y sin cumplir** en la cabecera del propio módulo (`btnBack (x3) … -> LECC.volverAlCapitulo`), que ahora lleva el aviso de lo que pasó. Se buscó **la clase y no el caso**: un barrido en los seis forks de toda llamada a nombres internos de los módulos que encapsulan (descartando `motor.js` y `voz.js`, cuyas funciones sí son globales) — **este era el único caso real**; los otros tres candidatos eran menciones dentro de comentarios. Verificado jugando antes y después, y la ✕ del **quiz normal** probada en los seis (nunca estuvo rota). Los seis quedan con la línea idéntica, incluidos 4° y 6°, donde hoy es inalcanzable pero mordería al darles mini-clases → **regla nueva nº 10** en este archivo | hecha | código |
| **A30** | **8 clips de voz de 3° quedaron huérfanos, y ya están generados (06/09).** Roberto oyó una voz distinta en una introducción de Lenguaje: no era un error de la voz sino **un texto sin clip** (cae al navegador, que suena distinto sin avisar). Los 3 diagramas de Lenguaje se agregaron el **05/09**, después de generar la voz en la Sesión 86, y nadie la regeneró. **No hizo falta escuchar nada para acotarlo**: `generar-voz-nivel.py <asig> --recuento` compara banco contra manifiesto sin gastar un centavo, y dio **1 en Historia, 4 en Ciencias, 3 en Lenguaje, 0 en las otras tres**. Generados (US$0,00 redondeado) y verificado que los tres vuelven a "0 faltan". ⚠️ **Regla que esto refuerza:** agregar contenido a un `lecciones.json` de 3° o 4° obliga a correr el `--recuento` de su asignatura — el hueco no da error, solo cambia de voz. ⚠️ **Y un gasto no autorizado que quedó anotado:** `auditar-voz-nivel.py --help` **no existe como bandera**, así que cayó al valor por defecto (`mat3`) y auditó de verdad los 89 clips que tenía pendientes (unos centavos, backlog real, no trabajo duplicado) | hecha | código |
| **A32** | ✅ **HECHA (06/09, Sesión 99).** **Lenguaje empieza a enseñar en los SEIS cursos: 33 lecciones** (17 mini-clases + 16 introducciones), y su banco de 8° queda completo (**844 preguntas, 26/26 OA** — cubría 15, y el propio `oa.json` declaraba ese recorte como si fuera la meta, así que `validar-oa-json.py` no tenía nada que detectar). ⚠️ **Lo que destrabó las mini-clases de Lenguaje NO fue la clase sino el REPORTE**: la regla de la Sesión 84 —*"Lenguaje nunca lleva mini-clase"*— existía porque su práctica le habría mandado al profesor un porcentaje junto a "escritura" midiendo si el alumno reconoce una definición (17 de 31 OA de 3° son producción o hábito). Con **`"mide": false`** el bloque de práctica **no llama a `registrarOA`**: enseña y hace practicar sin reportar, y el objetivo se sigue midiendo en las etapas normales del capítulo. ⚠️ **Se verifica con su control positivo o no se verifica nada**: 0 llamadas en la práctica **y más de 0 en una etapa normal** del mismo curso — sin la segunda mitad, una medición rota para todos se ve igual que la bandera funcionando (lección del `no_autorizado` universal, Sesión 73). Escritas por **3 agentes en una tanda** (regla de la Sesión 89), y los tres devolvieron **menos lecciones que capítulos**: 33 de 63, con los descartes argumentados contra el banco. ⚠️ **Un bug real y compartido en `normalizar-voz-nivel.py`**: el motor **sí** interpreta la negrita (`fmtLec` la pinta como `<b>`) pero **el normalizador de voz NO la quitaba**, así que los `**` habrían llegado crudos a Azure en 3° y 4°, los dos cursos con voz — 4° salió limpio pero **3° tenía 6 marcas**. Corregido con el **espejo exacto del regex del motor** (`_RX_NEGRITA`); verificado sobre los **1.245 segmentos locutables del proyecto: 0 con asterisco**, y ningún clip existente afectado, así que no hubo nada que volver a pagar. **Cableado**: `intro:` en 21 capítulos, la ruta en `LECC.init` y **`HAY_MINICLASES` encendida en 4° y 6°** — inocuo ahí (sus cuatro usos en `motor.js` están guardados por `esLecciones`/`capitulosMate`) pero **obligatorio igual**, porque el ✕ del quiz la consulta. **Y un defecto corregido de paso**: `nodoIntro` no seteaba `TRAS_LECCION`, así que al terminar una introducción el alumno salía del mapa a la pantalla de campaña. **Verificado jugando** en los seis: nodo 📘 primero, lección → quiz de 10, JUGADOR navega, cero consola y cero 404; **y con `lecciones.js` AUSENTE los seis siguen jugándose** (respaldo con las 6 funciones que el motor pide, byte a byte idéntico en los seis). `docs/estandar-miniclases.md` reescrito: la fila de Lenguaje ya no dice "nunca mini-clase" | hecha | contenido + código |
| **A33** | ✅ **HECHA (06/09, Sesión 100).** **Los dibujos de geometría llegan a las preguntas de 5° a 8°: 94 preguntas** (36 prisma · 14 triángulo · 10 círculo · 9 paralelogramo · 9 trapecio · 8 cilindro · 8 polígono). Los widgets ya existían pero vivían en el catálogo de las **mini-clases**, así que `pintaPregunta` intenta `renderVisual` y, si el `visual` declara **`kind`**, cae a `LECC.diagrama` — la caída simétrica de la que `montarDiagrama` hacía desde la Sesión 86. ⚠️ **`kind` es el widget y `tipo` la FORMA que ese widget recibe, y despachar por `tipo` mandaba una pregunta de área de triángulo al widget de PITÁGORAS**, que dibuja `a=3, b=4, c=5` y su cálculo **encima de un enunciado con otras cifras**; `{tipo:'prisma'}` simplemente no dibujaba nada. Se vio **jugando**: el conteo decía "1 SVG, sin errores" y las dos cosas eran falsas en su contenido. **Ningún dibujo escribe una cifra** —rotula las palabras «base», «altura», «radio», «diámetro»—, así que no puede contradecir el enunciado y los topes de `figura` (base 10, altura 6) quedan siendo solo proporción; `etiqueta:""` apaga el pie con la fórmula (en una pregunta de área **la fórmula ES el método que se mide**) y `cortes:false` apaga la partición del polígono por lo mismo. De **293 candidatas** quedaron 94: **el detector marca, la lista se leyó**, y se caen las que el dibujo contestaría (*"¿qué es una altura?"*, que el dibujo rotula), las que contradiría (prisma de base triangular, pentágono irregular, triángulo obtusángulo), las que nombran dos figuras y las que no usan ninguna medida que el dibujo marque. ⚠️ **Dos herramientas había que enseñarles el segundo catálogo**: `auditar-banco-nivel.py` daba 13/25/34/22 errores ("tipo desconocido") y **el informe de aprobación se habría bloqueado entero** con su aviso rojo, porque los 94 salían "no disponible" — las dos leen ahora los nombres de widget **del propio `lecciones.js`**, sin lista escrita a mano, y probadas rompiendo un banco a propósito. **Y de paso, 2 preguntas reescritas** (`mate5-oa21-19`, `mate7-oa07-25`): eran las 2 genuinas de 75 candidatas a multi-paso, y el paso intermedio se movió **a las opciones**. ⚠️ Quedan en `revisada:false`: cambió su texto, así que la firma anterior ya no les corresponde. **Costo medido en 375×667:** el quiz ya se desplazaba (4ª opción en 743 px) y el dibujo suma ~100 px, topado en 150 de alto. Verificado jugando los cinco widgets, **con `lecciones.js` ausente** (la pregunta se pinta sin dibujo y el quiz sigue), y las 151 lecciones intactas | hecha | contenido + código |
| **A34** | ✅ **HECHA (06/09, Sesión 101).** **El banco vuelve a estar entero: 16.625 de 16.625 preguntas y 151 de 151 lecciones**, tras firmar Roberto las 332 pendientes (330 de Lenguaje de 8° + las 2 de geometría reescritas) y las 21 lecciones. El diff fueron **353 marcas y nada más**: ningún archivo se reformateó, que es el formato canónico de la Sesión 85 haciendo su trabajo. ⚠️ **Y el 100% quedó dicho distinto que la última vez**: en la Sesión 93 se escribió *"cerrado del todo"* y seis días después había 332 sin firmar, así que los cinco documentos vivos dicen ahora que **es una foto, no un estado** y que el número **se vuelve a medir en cada orden 66 en vez de copiarse**. Se destrabó la **voz de las lecciones de Lenguaje de 3° y 4°: 41 clips, US$0,14**, con los 41 textos leídos antes de pagar (el normalizador toca solo 3, las de negrita, y las quita bien) y el camino completo verificado **espiando el constructor `Audio`** para ver qué clip pide el juego de verdad, no el "listo" del script. ⚠️ **Y el AI Brain: Google dejó de aceptar el login automatizado** (*"el navegador o la aplicación no son seguros"*), lo que invalida lo que `CLAUDE.md` daba por probado desde la Sesión 52 — se resuelve quitándole a Chrome `--enable-automation` y `navigator.webdriver`. Y **mi detección del login era la que fallaba**: exigía `notebooklm.google.com` y el flujo termina en `notebook.google.com`, sin "lm", así que el script se rindió con la sesión ya hecha. Los dos hallazgos van escritos **en la regla de la orden 66**, no solo en la bitácora  **Y de paso quedó listado el contenido sensible de Historia de 5°**, que estaba declarado en su `oa.json` y no en `docs/contenido-sensible.md` ni en `assets/js/sensible.js`: los capítulos **"Descubrimiento y conquista"** y **"La Colonia"** salían en el armador **sin marca**, o sea se podía repartir la conquista y la encomienda sin aviso. Pasó porque el documento nació con tres cursos y el fork de 5° se cableó antes de que copiarlo fuera costumbre. Un barrido de los 29 bancos confirma que **era el único hueco**; ⚠️ su primera versión acusó cinco porque leía el `oa.json` entero en vez de la nota, y me hizo reportar un falso positivo (`HI04 OA 01` está en el documento **dentro de la frase que dice que NO se marca**). Verificado en el armador: 5° pasa de 0 a 2 capítulos marcados, 8° intacto, y con el módulo ausente el armador degrada sin crashear | hecha | contenido + voz |
| **A35** | ✅ **HECHA (06/09, Sesión 101). El orden pasa a ser LIBRE por defecto** (`ORDEN_LIBRE`), decisión de producto de Roberto: VULPO se vende a colegios que pasan las unidades en su propio orden, así que obligar a jugar el capítulo 1 para llegar al 3 deja fuera justo al alumno cuyo curso está viendo el 3. **Todos los capítulos y sus etapas nacen abiertos; los jefes siguen pidiendo el camino** y ahí queda la meta. Buena parte ya existía —es el modo experimental de la Sesión 73—, que con esto **queda redundante** (se conserva pero no cambia nada, y `sincronizarModoCurso` deja de recargar). ⚠️ **NO era una línea:** `CAPS_ABIERTOS` gobernaba también dos candados **pedagógicos** —la mini-clase que abre su etapa y la lección que abre su nivel del Reto—, así que encenderla habría apagado *"aprender desbloquea el desafío"* sin ningún aviso; se partió en **`LECC_ABIERTAS`**, tercera vez que se corta una bandera por lo mismo tras `QA` (S41) y `MODO_ABIERTO` (S73). Las partidas ya guardadas se abren al leerlas (`abrirOrdenLibre`), conservando estrellas. Verificado contra una foto tomada ANTES: `o....` → **`oooo.`** en los seis, QA y `?solo=` sin cambio, candados pedagógicos en pie, y jugada de verdad una etapa antes inalcanzable | hecha | motor |
| **A36** | ✅ **HECHA (06/09, Sesión 101). Los enlaces de la landing pasan a ser MUESTRAS**: los 8 llevaban al **juego completo**, y con `ORDEN_LIBRE` recién encendido eso dejaba los seis cursos enteros abiertos a cualquier visitante. Ahora cada uno lleva `?solo=<capítulo>` — **una asignatura, una expedición**—, y el capítulo es el propio `DEMO_LIBRE` de cada curso, así que no hay dos criterios de "cuál es la demo" ni el visitante ve algo distinto antes y después de que cierre la puerta el 1/10. La fila dejó de decir *"Prueba el curso"* (promete de más con una unidad) por *"Prueba una unidad del curso"*. ⚠️ **La muestra no trae tienda, logros, perfil ni ranking** —el modo prueba oculta la barra—, a cambio de no guardar nada y de esquivar la puerta; revertir es cambiar 8 `href`. Verificado: los 6 cursos abren **1 capítulo** con su título correcto y la landing tiene **0 enlaces al juego completo** | hecha | landing |
| **A37** | ✅ **HECHA (07/09, Sesión 101). VULPO estrena correo propio: `contacto@vulpo.cl`.** Roberto contrató **Google Workspace** sobre el dominio que ya pagaba y **convirtió** su cuenta personal en vez de crear otra — medido primero que `vulpochile.app@gmail.com` era **solo el correo de contacto** y no la llave de nada (Supabase, NotebookLM y GitHub viven en su cuenta personal; Azure en una corporativa), y que el `@gmail.com` **sigue recibiendo**, así que ningún enlace repartido se rompe. Se configuraron **SPF, DKIM y DMARC**, que Google ofrece como opcionales y aquí no lo son: el canal de venta son correos a directores, y sin firmar un dominio nuevo es carne de spam. ⚠️ **El DKIM pide publicar la clave Y volver a la consola a darle "Iniciar autenticación"**; sin ese segundo clic Google no firma y no avisa. Verificado enviando de verdad a un Gmail: las tres en **PASS**. El correo se cambió en **17 lugares y no en los 14 contados** —la landing y el tutorial tenían dos cada uno, y faltaba `assets/js/motor.js`, el aviso del candado—; la bitácora histórica no se tocó. **No cambió** el login del panel del profesor, que es de Supabase y no un "entrar con Google" | hecha | infraestructura |
| **A38** | ✅ **HECHA (07/09, Sesión 101). El enlace de agenda de la landing**, pendiente desde la Sesión 51 y destrabado por Workspace (que quita el límite de una sola programación de citas). **`📅 Agenda una demo de 15 minutos`** en el cierre. Los dos canales **conviven a propósito**: WhatsApp arriba, porque en Chile es el canal natural de un director y pedir una hora al primer contacto es más fricción que valor; la agenda solo al final, cuando el visitante ya leyó todo. Ese botón decía *"Coordinemos una demo de 15 minutos"* y **abría un chat** — prometía una agenda y entregaba otra cosa. Verificado a 375 y 1280 px, con `rel=noopener` en los tres externos y **mirando la captura**, no solo contando | hecha | landing |
| **A39** | ✅ **HECHA (07/09, Sesión 102). Las estrellas fugaces caen del color de su elemento**, pedido de Roberto con una imagen de referencia. **Más flujo** (una cada **3–9 s**, antes 8–22) y **color sorteado** en cada una: oxígeno/nitrógeno rojo, hierro amarillo, sodio naranjo, calcio violeta, magnesio azul —el núcleo queda blanco, que es la parte más caliente—. Va como **una sola variable CSS `--meteoro`** que fija el JS, no una clase por color. ⚠️ **Y destapó un defecto vivo que el cambio empeoraba:** con el teléfono en **«sin animaciones»** la estrella se creaba igual y quedaba **QUIETA y visible 1,4 s** —el `@media` global apaga la animación pero no el elemento—; pasaba cada 15 s y habría pasado cada 6. Los seis forks ya no agendan ninguna. ⚠️ **El guard NO podía ser un `return`** como en la landing: ese mismo bloque **arranca el juego** unas líneas más abajo. El color se subió tras **compararlo mirando** —medía perfecto y se veía tímido—. **Segundo tramo:** Roberto vio que caían todas paralelas — y no era impresión: el vector era **siempre `(-d, 1.1d)`**, solo variaba la distancia. Ahora cada una sortea su **ángulo (30°–65°)** y su **largo de rastro (58–97 px)**. ⚠️ El rastro estaba clavado en `rotate(-48deg)` y tuvo que girar con el ángulo de su propia estrella: si se despega del camino se ve **peor que el defecto original**. Verificado con aritmética además de mirando (200 sorteos, 0 desalineados, desvío máx. 0,17°). De paso `cdp.mjs` ganó **`ev.medios(...)`**: el conductor fuerza `reduced-motion` para saltarse la intro, así que **nada animado se podía verificar** | hecha | estética |
| **A40** | ✅ **HECHA (07/09, Sesión 102). Los SEIS cursos tienen Vocabulario**, decisión de Roberto. Faltaban 4°, 5° y 6°: **240 preguntas** (4° con 2 áreas y 60; 5° y 6° con 3 y 90). **El alcance está medido, no supuesto:** el objetivo de *"determinar el significado de palabras nuevas"* existe en `LE03/LE04/LE05/LE06` y **desaparece en 7°**, que es exactamente lo que separa a los cursos con área de Lenguaje de los que no. ⚠️ **Del de 6° salieron "pubertad" e "higiene corporal"** (`CN06 OA 05/06`): el armador marca **por código de OA** y los `VOC-*` son compartidos entre cursos, así que no hay forma de marcar solo ese módulo — ese contenido vive en su capítulo de Ciencias, que es donde sí se marca. ⚠️ **Y destapó un defecto de motor preexistente:** con `ORDEN_LIBRE` la última etapa nacía cerrada porque el código asume **por posición** que la última es el jefe, y Vocabulario y Lectura **no tienen jefe** — en 5° y 6° Matemática habría nacido bloqueada. Medido: de las 150 expediciones de los seis cursos, **exactamente 8 no terminan en `BOSS`** (los 6 vocabularios y las 2 lecturas). Ahora se pregunta si la última etapa **es** un jefe. Contra una foto tomada ANTES: **142 expediciones idénticas, las 8 esperadas abiertas, ninguna más**, y `?qa=1` sin moverse. Su aprobación y su voz se cerraron el mismo día: ver **A41** y **A42** | hecha | contenido + motor |
| **A41** | ✅ **HECHA (07/09, Sesión 102).** **El tablero decía "no queda nada por revisar" con 150 preguntas sin firmar.** De las 240 del Vocabulario se aplicaron **90**, y el patrón —60 en 4°, 30 en 5°, 0 en 6°— era la huella del defecto y no la de una revisión interrumpida: `armarCola()` deduplicaba **por código de OA a secas**, y los transversales **no llevan el nivel en su código**, así que `VOC-CIEN` es el mismo texto en los seis cursos. La cola mostraba tres tarjetas y aprobarlas marcaba solo la primera carpeta pendiente. ⚠️ **Fallo MUDO**, de la familia del `pg_cron` sin agendar: se ve idéntico a que esté todo hecho, y **crece con el proyecto** —mientras solo se aprueba la cola encoge y disimula; el día que se agrega contenido después de haber aprobado todo, deja el modo permanentemente mudo—. Segundo camino al mismo síntoma tras el clamp del índice (Sesión 74). La clave pasa a ser **carpeta + código** (`data-carpeta` en el DOM), y la semilla de la muestra también, o `VOC-CIEN` de 4° y 5° sorteaba las mismas ocho posiciones. **Alcance medido:** de todos los códigos del banco solo los **cuatro `VOC-*`** viven en más de una carpeta. Verificado con el contraste sobre el mismo tablero (**cola vieja 3 · nueva 5**) y recorriendo las cinco desde la interfaz. ⚠️ **Y con el control positivo**, que es lo único que distingue un banco completo de un detector roto: desmarcar a mano una pregunta en 4° y otra en 6° con el mismo código, y comprobar que la cola encuentra **las dos**. Queda como procedimiento en `docs/aprobacion-pedagogica.md`. **Banco: 16.865 de 16.865 y 151 de 151** | hecha | herramienta |
| **A42** | ✅ **HECHA (07/09, Sesión 102). La voz del Vocabulario de 4°: 305 clips, US$0,24.** Dos cosas antes de pagar: los **300 textos pasaron por el normalizador y 0 cambian** (prosa pura, como los libros — la familia del *"enero"* y el menos que desaparece no puede ocurrir ahí, así que no queda nada que escuchar después); y ⚠️ **los cuatro `caps` de 4° estaban SIN VERIFICAR**, escritos por extrapolación cuando el fork no existía. Contrastados contra `4to/index.html`: los cuatro correctos, incluida la duda de Ciencias (preguntas `cien4-`, capítulo `cie4-`). ⚠️ **Y faltaba el cableado**: `VOZ_DIRS` de 4° no incluía `voc4/`, así que el juego habría caído a la voz del navegador **sin decir nada** (el defecto de la Sesión 98). Verificado **jugando y espiando el constructor `Audio`**: el 🔊 pide `voc4/5bff98585cd53a19.mp3` → 200 `audio/mpeg`. Peso remedido: sitio publicado **729 MB** | hecha | voz |
| **A43** | ✅ **HECHA (07/09, Sesión 103). Todos los capítulos de Historia llevan introducción: las 33, en los seis cursos** (eran 12). Regla de Roberto: cada una abre diciendo **en qué siglo estamos, entre qué años pasa y en qué lugar del planeta**. ⚠️ **Solo 12 de los 33 son históricos**: los otros 21 son de geografía o de formación ciudadana, y ahí el siglo y los años se responderían con "hoy, siglo XXI", que es relleno — decisión de Roberto: **el lugar siempre, el tiempo cuando fecha algo de verdad** ("la Constitución actual se escribió en 1980"). Las **12 que ya existían se rehicieron** y quedan `revisada:false`: cambió su texto. El marco va como **widget** (`kind:'marco'`) y no redactado, porque escrito a mano el día que alguien lo omita **no lo nota nadie**; lo comprueba `scripts/revisar-marco-historia.py`, escrito ANTES que el contenido. Estándar en `docs/estandar-introduccion-historia.md`. **Tres defectos encontrados, dos vivos en producción:** el widget `tiempo` escribía los años con separador de miles (**"1.492"**, "1.810", "1.789" en 7° y 8° — un año no lleva punto; solo una cantidad como "300.000 a.C."), y sus **rótulos de dos líneas se leían INVERTIDOS** hacia arriba ("América / Colón llega a"). Los dos invisibles para cualquier conteo —los elementos existen y no desbordan— y **vistos en la captura**; como es presentación y no dato, 7° y 8° no pierden su firma. ⚠️ **Y el tercero lo dejó pasar mi propio verificador:** en 4°, 5° y 6° el fork **no cargaba** el archivo de Historia en `LECC.init`, así que el nodo 📘 no se dibujaba —el capítulo se juega igual, sin introducción— y el chequeo daba **verde** porque miraba el dato y no el camino que lo usa. Cerrado y **probado rompiéndolo a propósito**. Verificado **jugando en los seis** y con un save sembrado: el nodo nuevo **no corre el avance guardado**. Los seis agentes aportaron **seis fugas** que regalaban respuestas del banco, un **desajuste de fechas** en 6° (ficha en 1830, seis preguntas comparan desde 1810) y los dos bugs del widget; el de 8° midió la similitud de sus frases contra las de 5° para no repetir (de 0,82 a 0,54). **Firmadas el mismo día: 172 de 172** | hecha | contenido + código |
| **A44** | ✅ **HECHA (07/09, Sesión 104). El camino enseña→desafío llega a los SEIS cursos.** Se escribieron las **51 mini-clases de Matemática de 4° y 6°** (27 + 24) y se convirtieron sus dos campañas —`esLecciones:true` + `capitulosMate`—, más las **10 introducciones de Ciencias** de esos cursos. Matemática pasa de 89 a **140 mini-clases**, una por OA en los seis cursos, y el proyecto de 172 a **233 lecciones**. ⚠️ **Lo primero fue corregir esta misma tarea: Lenguaje NO tenía trabajo pendiente** — sus 13 capítulos sin lección están así por decisión y con argumento escrito en `docs/veredicto-historia-lenguaje.md`. **Cinco defectos de widget encontrados, los cinco vivos en producción**: `posicional` recortaba a 999 (la lección de 5° enseñaba 507.320 y el dibujo decía 999), `balanza` dejaba fichas fuera del platillo (3°, 5°, 7°, 8°), `transformacion` cruzaba su pie con el eje, la etiqueta de `esquema` no se partía y **24 estaban cortadas en 8 asignaturas y 6 cursos**, y el normalizador de voz **no traducía el signo de dividir** — 36 preguntas de 4°, que lleva voz, se locutaban con el símbolo crudo. Los cinco son presentación y no dato, así que las lecciones aprobadas no pierden su firma. ⚠️ **Y uno que NO se arregló**: las fichas de la segunda fila de la balanza cruzan la bandeja 4 px, y mirado eso es que **se apoyan** — un número que marca algo no es un defecto. ⚠️ **El defecto de la ruta de `LECC.init` mordió por tercera vez en el día, y fue el propio guard**: buscaba la ruta en todo el archivo y el fork la nombraba en un comentario. Ahora mira dentro de `rutas:[…]`. Verificado jugando: 9 nodos en la unidad de 4° con la etapa 1 en «Termina las lecciones». **Las 61 quedaron firmadas el mismo día: 233 de 233 lecciones** | hecha | contenido + código |
| **A45** | ✅ **HECHA (07/09, Sesión 103).** **El modo de aprobación rápida decía "no queda nada" con 33 lecciones sin firmar.** El botón **"⚡ Aprobar por muestreo" recorre SOLO preguntas** —se construyó en la Sesión 70 para eso— y las lecciones entraron al circuito quince sesiones después, con su propio camino (la sección 📘 y su casilla): **los dos nunca se conectaron**. Su letra chica decía *"objetivos con preguntas pendientes"*, literalmente cierto y que nadie lee. ⚠️ **Segunda vez el mismo día que ese modo anuncia que no queda nada teniendo trabajo pendiente**, tras la cola dedupeada por código (A41), y las dos veces lo destapó Roberto usándolo. **Lo que NO estaba roto:** `marcarAprobadas` sí cuenta lecciones, así que las secciones de Historia salían arriba con su botón a mano — el tablero estaba bien y el camino rápido era el que no llevaba ahí. Ahora el cierre dice **"Las preguntas están listas · quedan N lecciones"** con el desglose por curso y un botón **"📘 Ir a las lecciones"** que deja la primera desplegada; y cuando de verdad no queda nada, lo afirma de las dos cosas. Verificado **con control positivo**: desmarcar una lección a propósito y comprobar que la detecta. **Las 33 firmadas el mismo día: 172 de 172 lecciones y 16.865 de 16.865 preguntas** | hecha | herramienta |
| **A46** | ✅ **HECHA (07/09, Sesión 105). La voz de Matemática de 4° queda completa: 184 clips, US$0,47.** Eran 42 por el signo de dividir, y al leerlos antes de pagar aparecieron **dos defectos más del normalizador y una tanda entera sin generar**. ⚠️ **1 · El arreglo del signo no cubría la incógnita:** exigía dígitos a los dos lados, así que *"617 ÷ ___ = 617"* se quedaba con el símbolo crudo — el mismo defecto que ya tuvo el guion (Sesión 56), cuyo arreglo vive dos líneas más abajo. Medido sobre las **329 apariciones del símbolo en los 32 bancos**: 259 las cubría la regla vieja, 68 la nueva, y las 2 restantes son rótulos de diagrama que no se locutan. ⚠️ **2 · La regla del conteo salteado —la del "enero"— pegaba en el castellano corriente:** aceptaba cualquier par de números, así que *"Parto el 68 en 40 y 28"* quedaba *"sesenta y ocho en cuarenta **y 28**"*, convirtiendo dos de los tres. Acotada a **el mismo número a los dos lados**, que es lo que define un conteo salteado: 76 coincidencias legítimas contra 24 que eran *"divido 12 en 4"* o *"cuántas veces cabe el 7 en 238"*. ⚠️ **3 · Y las 27 mini-clases de Matemática de 4° estaban MUDAS**: se escribieron ayer y su voz nunca se generó, así que el 🔊 caía a la voz del navegador — el fallo mudo de la Sesión 98. Roberto autorizó cerrarlo todo. **Regresión medida antes de tocar nada:** sobre **75.895 textos**, cambian 44 y los 44 llevan el signo. Verificado **jugando**: el texto que la mini-clase muestra resuelve a `mat4/574f3f0f1b5a1cbf.mp3` → 200 `audio/mpeg`, y los clips con operación pesan **14 KB contra 9 KB** de un número suelto. ⚠️ **No se auditó con STT a propósito**: el transcriptor colapsa las operaciones entre números (documentado en la Sesión 101 con "menos"), así que gastaría sin resultado concluyente. **Y se documentó un gotcha nuevo**: el almacén `dev/auditoria-voz-*.json` se indexa por texto igual que el manifiesto, así que un clip regenerado conserva la transcripción del audio que ya no existe — encontrado con *"Divido 12 en 4"*, cuyo almacén afirmaba *"Divido dos en cuatro"* | hecha | voz |
| **A47** | ✅ **HECHA (07/09, Sesión 106). El panel del profesor se ordena para el computador, y estrena la tendencia semanal.** Roberto pidió ideas y eligió tres, más una condición: *"el panel lo revisará en el computador"*. **El ancho** crece en dos escalones (880 px y **1.180** desde 1.240) y se gasta en densidad: los alumnos van en **columnas** (22 alumnos pasan de 22 filas a **8**) y cada objetivo cabe **en una línea** (un mapa de 60 pasa de tres pantallas a una y media, sin recortar texto); bajo 900 px todo vuelve a apilarse. **Administración queda plegada** en un solo desplegable con *Crear curso* adentro —eran ~800 px de formularios que se usan una vez al año, bajo una tarjeta de curso de 130—, y no se cierra sola tras cada acción. **Tres pesos de botón** con la regla *un primario por bloque*: dorado la acción del bloque, con borde el resto, **en texto lo que no se deshace** (*Limpiar perfiles*, *Reiniciar mediciones*, *Cerrar sesión*, que antes pesaba lo mismo que *Crear curso*). **Y la tendencia semanal**: `kimun_prof_tendencia(curso)` es la primera lectura de las fotos que `kimun_foto_semanal` tomaba cada domingo **desde la Sesión 36 sin que ninguna pantalla las mirara**. Titular sin desplegar (*"Esta semana · 240 respuestas · 68% de acierto (antes 73%)"*) y franja de 8 semanas. ⚠️ **NO grafica el porcentaje del mapa**, que es de primer intento y queda congelado, así que entre dos semanas da el mismo número y se vería roto sin estarlo: muestra la **diferencia de los acumulados**. Tres casos se dicen con todas sus letras — sin dos fotos no hay comparación; un delta negativo es *"se reiniciaron las mediciones"* y no una caída; y bajo 10 respuestas va atenuado con *"muy pocas para concluir"*. **Cuatro defectos encontrados MIRANDO**: (1) abrir *Ver avance* **saltaba al final de la página** —`aviso('')` centraba un mensaje vacío que vive al pie y deshacía el `irArriba()`—, vivo en producción; (2) con dos fotos pero sin actividad decía *"todavía sin comparación"*, o sea que faltaba historia al sistema; (3) los `select` se estiraban a 1.100 px y *"Nivel del curso"* etiquetaba al campo de arriba; (4) en el teléfono la franja arrancaba en la semana más vieja. **El error de método:** la fila del alumno salía vacía y parecía un defecto grave del panel — era **el doble de Supabase**, escrito de memoria (`nombre` donde el servidor devuelve `alumno`), y al revisarlo contra las firmas aparecieron **cinco funciones más mal simuladas**. *Cuando el resultado sorprende, el primer sospechoso es la prueba.* Verificado a 1.440 y 375 px jugando con clics: sin desborde, **cero consola y cero red**, los seis casos de tendencia correctos, y **con la función SQL ausente el mapa se pinta igual**. `cdp.mjs` ganó `ev.escritorio` (sin él las media queries de escritorio no se podían verificar). ✅ **Esquema aplicado y comprobado el 07/09** (ver [`docs/aplicar-schema.md`](docs/aplicar-schema.md)) | hecha | código + backend |
| **A48** | ✅ **HECHA (07/09).** **El armador gana un botón por asignatura y el modo prueba pasa a dos niveles.** En el armador, cada asignatura trae **Todos / Ninguno** que *alterna*: desmarcar una materia entera es tan frecuente como marcarla —el caso típico es "todo menos Ciencias"—. Cada grupo vive en su propio `.arm-grupo`, así el botón sabe cuáles son SUS casillas sin depender de la posición en el DOM. ⚠️ Marcar por código **no dispara `change`**, así que el enlace se recalcula a mano; sin eso mostraba el anterior. El **modo prueba** (`renderListaPrueba`) va ahora en dos niveles como la pantalla principal: **asignaturas y libros afuera** —el libro es un grupo propio, no un capítulo colgando de Lenguaje— y dentro de cada una sus capítulos. Antes era una lista plana donde un enlace de varias asignaturas eran veintitantas tarjetas sin agrupar. ⚠️ **Con un solo grupo se entra directo**, así los enlaces ya repartidos (incluidos los ocho de la landing) se ven igual que antes; y el "← Volver" del segundo nivel recibe **su propio onclick**, porque el del fork lleva a `scr-expediciones`, la fuga al juego completo que cerró la Sesión 41. Los dos cambios viven **solo en `assets/js/motor.js`**: ningún fork se tocó. Verificado jugando con clics en 3° (nivel 1 con Historia/Ciencias/**Lectura**, entrar, volver, y desde el mapa de un capítulo volver a SU asignatura) y **regresión en los seis cursos** —motor vivo, JUGADOR navega, armador con sus grupos y botones—, cero consola y cero red. ⚠️ **Mi prueba estaba mal otra vez**: buscaba el id crudo en el enlace, y el armador los codifica en el token `?m=` | hecha | código |
| **A49** | ✅ **HECHA (08/09, Sesión 109). Las cuatro ideas que salieron de mirar el panel con un doble quedan cerradas.** ~~(a) bajar el registro visual~~ (A50) · ~~(c) imprimir~~ (A53, el pulso) y ahora el informe de cierre. **(a) La franja de "qué necesita tu atención"** abre el panel: hasta hoy era una lista de cursos plegados —correcta, ordenada y muda—, y para enterarse de algo había que abrir un curso, entrar a Ver avance y leer. ⚠️ **La primera versión ponía una línea POR HALLAZGO y salieron NUEVE de dos renglones: 830 px en un teléfono de 780**, o sea que el primer curso quedaba a **1.300 px de scroll** — se pidió ver mejor y la franja tapaba justo lo que había que ver. Es el defecto del banner de instalación (Sesión 77) otra vez, y **se vio MIRANDO la captura**: el conteo decía "sin desborde, cero errores". Ahora es **una línea por CURSO** (4 como máximo, más "y N cursos más"), y agrupar no pierde ninguna acción porque el mapa, la participación y el refuerzo **viven los tres en Ver avance**; el primer curso queda a **489 px**. Ordena por lo que falta configurar primero y después por proporción de alumnos fuera —proporción y no número: 20 de 20 es un curso detenido y 5 de 22 una semana normal—. **(b) El titular del curso** pasa a *"Participación · 17 de 22 esta semana · 77% ↓"*, con el `title` diciendo de dónde sale cada número: ⚠️ la semana en curso de `visto` y la anterior del **delta de XP entre dos fotos**, que es la única fuente de participación pasada que existe (`perfiles.visto` guarda solo la última entrada). Se dice en la interfaz en vez de esconderlo, y el primer número NO se toca: tiene que seguir siendo el mismo que el pulso y el bloque de participación. Las dos salen de **`kimun_prof_resumen()`**, que ⚠️ **nace para BAJAR las llamadas y no para subirlas**: reemplaza la consulta de participación *por curso* que hacía la lista, y no devuelve un solo nombre de alumno | hecha | código + backend |
| **A50** | ✅ **HECHA (07/09, Sesión 106). El panel deja el registro del juego.** La decisión se tomó **mirando**: tres copias temáticas del panel con los mismos datos y el mismo ancho —el actual, un sobrio oscuro y uno claro—, comparadas lado a lado. Roberto eligió el **sobrio oscuro**, y pesó que **la landing ya usa Inter desde la Sesión 51 por el mismo motivo**: con esto las dos caras adultas hablan igual y **el juego se queda como está** —es una decisión por PÚBLICO, no una migración a medias—. Inter en vez de Titan One y Nunito, `body` de peso **800 a 400** (era lo que hacía que el texto de un objetivo pesara igual que su porcentaje), fondo neutro, radios de 16 a 10 px y **el color reducido a donde informa**: los tres semánticos se conservan porque dicen si un objetivo va bien, regular o mal. ⚠️ **La palanca son las variables CSS**, no perseguir reglas: casi todo el color sale de siete, incluidos los estilos que el JavaScript escribe inline. **Cuatro defectos que aparecieron al implementarlo y NO estaban en la maqueta**, los cuatro vistos mirando: la jerarquía dentro del curso estaba **invertida** ("Agregar alumno" lleno y "Ver avance" en gris, al revés de lo que se hace cada semana); **tres desplegables en tres colores** para cosas del mismo rango; **el violeta como texto no llegaba al mínimo de contraste** (3,4:1) y quedó reservado para acciones, con su tono elegido MIDIENDO (blanco encima pasa de 4,44 a 5,06); y la tarjeta de ingreso estirada a 1.180 px. Verificado a 1.280/1.440 y 375 px, con los seis casos de la tendencia intactos, cero consola y cero red, y comprobando que **no quedan restos del registro anterior** —un tema a medias es peor que no cambiarlo— | hecha | código |
| **A51** | ✅ **HECHA (07/09, Sesión 106). Lo que le falta al que ve las CUATRO asignaturas.** Roberto pidió dos cosas para el Profesor Jefe y el SuperUsuario, dejando claro que *"para el profe de asignatura está perfecto"* — y las dos **solo aparecen con más de una asignatura a la vista**. **(1) Ranking general**, con `kimun_prof_ranking_general(curso,minimo)` nueva: una pestaña "General" delante de las cuatro. ⚠️ **El promedio es SIMPLE, no ponderado por respuestas**, y ahí está la decisión: "el promedio de las cuatro" significa que cada una pesa igual; sumar todos los aciertos dejaría que **la materia que más se jugó** decidiera el número del alumno. Por eso cada fila dice **"4 de 4 asignaturas"**: un 78% sobre dos y un 72% sobre cuatro no son comparables. **(2) El mapa dice de qué asignatura es cada objetivo y se puede agrupar.** ⚠️ El agrupado **no reemplaza los tres bloques: los SUBDIVIDE** —cambiar el corte a "una sección por asignatura" perdería la distinción entre reforzar / van bien / pocos datos, que es lo que el panel aporta—, así quedan las dos lecturas: primero qué hacer, después de qué materia. La etiqueta va como texto atenuado **sin color propio**, porque el color de esa tabla ya significa rendimiento. **Dos defectos:** la ficha de un alumno llamaba `filas.map(filaAvance)` y `map` pasa el **índice** como segundo argumento —la etiqueta habría aparecido desde la segunda fila—, atajado enumerando los llamadores ANTES de cambiar la firma; y **el "Cargando…" arrastraba la página 81 px** porque su `scrollIntoView` suave seguía animando tras el `irArriba()`, así que los mensajes de *estoy trabajando* dejaron de desplazar y los errores y éxitos siguen haciéndolo. ⚠️ **Su control negativo falló al primer intento** dando 0 en los tres casos: con el panel plegado la página cabía en el viewport y no había a dónde desplazarse — era la prueba, no el producto. ✅ **Esquema aplicado y comprobado el 07/09** | hecha | código + backend |
| **A52** | ✅ **HECHA (07/09). Quién entra y a qué cursos.** Pedido de Roberto: *"cuando el profe entre a su módulo debe ver su nombre, su correo y sus cursos en orden ascendente, y que se pueda colapsar la info de cada curso"*. El encabezado pasó de una línea que además **enumeraba todos los cursos con su rol** —larguísima con varios, y describiendo cursos que estaban 300 px más abajo— a nombre + rango + **correo**; el rol se mudó a la tarjeta de cada curso ("Tú: Profesor jefe"), que es donde se lee junto a lo que describe. ⚠️ A un Admin **sin membresía** no se le muestra: ahí el servidor le devuelve todas las asignaturas del nivel y enumerárselas no diría nada de su rol. **Orden ascendente por nivel y luego nombre** —el servidor ordena solo por nombre, y así "8A Prueba" caía antes que "Quinto B"—, con los cursos sin nivel al final. **Contraer/Expandir todo** (un botón que alterna, como el Todos/Ninguno del armador) y **al cerrar un curso se cierra lo de adentro**, que era lo que hacía que reabrirlo lo devolviera con la lista de alumnos desplegada. ⚠️ **Y un defecto propio visto MIRANDO:** el chip de rol dejaba cada tarjeta cerrada en cinco líneas apiladas —o sea pedían ver mejor y yo engordaba el curso—, así que en computador las cuatro metalíneas pasaron a **una sola fila** y cuatro cursos cerrados bajaron de llenar la pantalla a media. Casos de borde probados: profesor **sin nombre** (el correo hace de nombre y no se repite) y **un solo curso** (sin barra) | hecha | código |
| **A53** | ✅ **HECHA (07/09). El pulso del colegio: la pantalla que le faltaba a la dirección.** Roberto pidió *"una mejora ejecutiva… que les guste a los profesores y al director"*. Tres agentes midieron el panel, el esquema y lo comercial, y el hallazgo que ordenó todo es que **el director NO tiene panel**: un SuperUsuario ve lo mismo que un profesor, con más cursos, y tiene que entrar uno por uno —la comparación entre cursos quedó fuera de alcance en el spec de 2026-08-17 y nadie volvió sobre eso—. La vista nueva muestra **una fila por curso con la COBERTURA DEL PROGRAMA y la PARTICIPACIÓN**, y es imprimible en una hoja. ⚠️ **Cobertura y participación, y ningún porcentaje de acierto: esa es la decisión.** Un tablero que ordene cursos por rendimiento **es un ranking de profesores con otro nombre** —lo advierte `CLAUDE.md` desde la Sesión 24— y encima impreso. Cobertura y participación son **hechos**; el acierto es juicio, y se mira dentro de cada curso. Por eso la restricción vive en la **firma** de `kimun_prof_pulso()`, que no devuelve porcentajes ni nombres, y no en el CSS: no se resuelve desde el cliente porque `kimun_prof_participacion` **devuelve los nombres de los alumnos**, así que la pantalla que promete no mostrarlos los estaría descargando igual. Las cabeceras son texto plano **sin ordenar por columna**, y no hay barra de progreso: una barra rellena es un ranking dibujado como geometría. **El denominador es honesto y se verificó**: VULPO tiene contenido para todos los OA del año salvo uno de Lenguaje de 3°, así que "31 de 86" significa "faltan 55 por tocar" y no "VULPO no los tiene". El desglose por asignatura va porque *"43 de 93"* no dice qué hacer y *"Ciencias 0/14"* sí. ⚠️ La cobertura cuenta **solo códigos con forma curricular**: `VOC-HIST` y `AF-T*` mapean a asignaturas de 8° y no están en su `oa.json`, así que sin ese filtro un 8° diría "23 de 22" (comprobado sembrándolo en el doble: dice 62 de 80, no 65). **Cuatro defectos vistos mirando**, con el conteo diciendo "sin desborde, cero errores": el título salía **dos veces** en el papel, las columnas mal repartidas dejaban 120 px de aire, el botón se partía en dos líneas, y en el teléfono los anchos en % partían el código del curso en dos. ✅ **Esquema aplicado y comprobado el 08/09**: `kimun_prof_pulso` pasó de **404** a **400 `no_autorizado`** —el cambio de estado prueba a la vez que existe y que su portero rechaza—, con el control negativo en pie (una función inventada sigue dando 404) | hecha | código + backend |

| **A54** | ✅ **HECHA (08/09). Las unidades tienen fechas, y el mapa dice cuándo.** Pedido de Roberto: *"el profesor de asignatura necesita poner fechas de inicio y término a sus unidades… no sirve mostrar avances en diciembre de asignaturas que se pasaron en abril"*, con la pregunta de fondo de **cómo guardar una foto al cierre si los niños siguen jugando**. ⚠️ **La respuesta es que la foto YA se está tomando:** `dominio_semanal` guarda el detalle alumno×objetivo cada domingo desde la Sesión 36, así que el cierre se **reconstruye** —las fechas se pueden poner después, incluso a fin de año, sin ningún botón de "cerrar unidad" que se pueda olvidar—. Y media parte del problema ya estaba resuelta sin saberlo: el porcentaje del mapa es de **primer intento** y no se mueve, así que un objetivo de abril ya mostraba el número de abril; lo que faltaba era **fecharlo y agruparlo**. **Fase 1:** `kimun_prof_dominio` devuelve `ultima` y `recientes`, y cada fila dice *"21 jugaron este mes"* o *"sin movimiento desde junio"* — ⚠️ **los DOS números y no solo la fecha**, porque `dominio.actualizado` se sobreescribe y un objetivo de abril re-jugado en noviembre diría "noviembre"; con `recientes` se distingue "el curso está en esto" de "un niño repasó". Más **agrupar por unidad**, gratis porque los 32 `oa.json` ya declaran `unidades[].oa` (medido: cero objetivos huérfanos). **Fase 2:** tablas `unidades_plan` y `unidades_plan_log`, la UTP carga y el profe ajusta **con justificación obligatoria** al mover una fecha ajena, y la propuesta automática sale de las fotos —⚠️ mirando cuándo **creció** el primer intento y no cuándo era mayor que cero, porque `resp_1` nunca baja y `max(semana)` habría devuelto siempre la semana actual—. ⚠️ **El historial completo lo ve la UTP, por decisión de Roberto y sobre advertencia escrita**: es material de evaluación docente, y si mover una fecha se siente como confesar un atraso, los profesores dejan de moverlas y los informes vuelven a estar mal fechados. **Dos defectos vistos mirando:** la etiqueta de "cuándo" quedaba **fuera del recorte a dos líneas** del texto —presente en el DOM, invisible, con el conteo diciendo 14 de 14— y, ya movida, en el teléfono **aplastaba la barra de progreso a CERO píxeles** sin que el "sin desborde" lo delatara. ✅ **Esquema aplicado y comprobado el 08/09**, y con **tres** comprobaciones: las cinco funciones nuevas dan **400 `no_autorizado`** mientras una inventada da **404**; **la firma se probó de verdad** llamando `kimun_prof_plan_fijar` con sus siete parámetros por nombre —PostgREST resuelve así, y una firma distinta habría dado `PGRST202`—; y las dos que cambiaron de firma siguen respondiendo | hecha | código + backend |
| **A55** | ✅ **HECHA (08/09, Sesión 109). El informe de cierre de unidad y de fin de año.** Cierra la fase 3: con las fechas puestas (A54) y las fotos semanales, esta es la pantalla que las usa. Cada unidad con término pasado **y una foto que lo cubra** gana un **📸 Cómo quedó**, y abajo va **📅 Informe de las unidades cerradas**, imprimible en A4. ⚠️ **Una función para tres usos**: `kimun_prof_dominio_foto(curso, semana)` es el mapa leyendo la foto en vez del acumulado vivo, con la misma forma y el mismo filtro por asignatura, así que sirve para el cierre de una unidad, para el informe de fin de año y para cualquier *"cómo estábamos en junio"* — una función por cada uno habría sido tres veces el mismo SQL. Más `kimun_prof_semanas(curso)`, que dice **hasta dónde llega el historial**. ⚠️ **La comparación con hoy va A LA VISTA y no escondida, porque es el argumento entero**: un objetivo que cerró en 78% y hoy marca 61% no bajó — entraron alumnos que lo tocaron por primera vez en otra unidad y su primer intento se sumó al promedio de abril. Ver los dos números juntos es lo que lo explica. ⚠️ **La foto elegida es la PRIMERA POSTERIOR al término**, no la anterior más cercana: si la unidad terminó un martes, la foto del domingo anterior no incluye el lunes ni el martes. El costo es incluir hasta seis días en que el curso ya está en otra unidad, y como el porcentaje es de primer intento ese ruido es mínimo. ⚠️ **No devuelve `ultima` ni `recientes`**: los dos salen de `dominio.actualizado`, que se sobreescribe, así que en una foto pasada dirían cuándo se tocó por última vez HOY — un dato que no existe en la foto no se rellena con el de hoy. **Y el límite que hay que decirle a un colegio antes de prometerlo sigue en pie**: la primera foto real es del **30/08/2026**, así que de una unidad cerrada antes no se reconstruye nada; por eso el pie del informe dice desde cuándo hay historial. Para 2027 habría el año completo desde marzo | hecha | código + backend |
| **A56** | ✅ **HECHA (08/09).** Los dos pegados aplicados. ~~**(1) El portero de `kimun_prof_resumen`**~~ ✅ **re-pegado el 08/09 y comprobado**: pasó de **200 `[]`** a **400 `no_autorizado`**, estable, con su control negativo (un parámetro inventado da **404 `PGRST202`**, o sea que el 400 no es un eco) y su hermana `kimun_prof_listar` respondiendo lo mismo, que era el patrón al que había que volver. **Con eso el backend no tiene nada pendiente.** **(2) `seed-anio-8vo.sql`** (fuera del repo, con su `LEEME`), que fabrica **26 fotos semanales del 15/03 al 06/09/2026** en el 8° de prueba para poder mostrarle a un colegio lo que VULPO hace con un año de uso. ⚠️ **Reemplaza el desempeño de `CUR-BA04`** pero **no borra ningún perfil**, que es toda la diferencia con `seed-CUR-BA04.sql` —ese empieza con `delete from perfiles` y las fotos cuelgan con `ON DELETE CASCADE`, así que se las llevaría—. Los dos **no son intercambiables**. ✅ **Pegado el 08/09 por Roberto.** ⚠️ **El asistente NO puede comprobarlo**: todo lo que lee `dominio_semanal` exige sesión de profesor y en el equipo solo está la clave pública. Se ve abriendo el panel — la tendencia con historia hasta marzo y el botón 📸 **Cómo quedó** en las unidades cerradas | hecha | Roberto |
| **A57** | ✅ **HECHA (08/09, Sesión 110). Lo que se está pasando arriba, lo terminado al historial.** Pedido de Roberto mirando la ficha de una alumna: *"en el análisis de los alumnos, en primer lugar debe mostrar sus resultados de las unidades abiertas; las cerradas deben quedar como historial, por asignatura"*, y después la corrección que lo completó: *"igual debe haber un indicador de lo que está abierto o pasándose en clases y lo que ya está terminado"*. ⚠️ **Esa segunda frase no era un detalle: eran TRES estados y no dos**, y se ve en su propio 8° de prueba — U3 corre del 20/07 al 25/09 y U4 arranca el 28/09, así que el primer corte ponía al lado un objetivo urgente y uno que no ha visto nadie. Quedan **📖 En clases ahora** (arriba, abierto), **🕒 Todavía no empieza** (plegado) y **📚 Historial · unidades terminadas** (plegado, por asignatura y dentro por unidad en orden curricular con su tramo del año), **en las dos vistas** —curso y alumno—, más un **sello por unidad** en la planificación del año, que listaba las 17 unidades con sus fechas y no decía cuál corría. ⚠️ **SIN PLANIFICACIÓN LA PANTALLA QUEDA IDÉNTICA A ANTES** —0 sellos, 0 secciones, las mismas filas, verificado desde el arranque—: las fechas son opcionales y hoy casi ningún curso las tiene llenas, y una feature que le cambia la pantalla a quien no la usa es una feature que estorba. Por eso *sin fecha de término NO está cerrada*. ⚠️ **Los iconos no son puntos de color**: en este panel el verde ya significa rendimiento, así que un 🟢 se leería como "esta unidad va bien". ⚠️ **El historial no lleva los tres bloques** (Para reforzar / Van bien / Pocos datos): decir "refuerza esto" sobre algo ya cerrado es el problema que esto resuelve. **Y de paso se cerró un defecto que llevaba DOS SESIONES vivo:** los tres botones "Agrupar por" se dibujaban en la ficha del alumno **sin responder a nada** —`verAvanceAlumno` nunca llamaba a `conectarAgrupar`—, y un control que se dibuja y no hace nada no da ningún error | hecha | código |
...
para los tres**, y ahora también el banco de los tres.

> **Avance real al 05/09: 8.490 de 8.490 (100%) — el Bloque B queda con el banco de sus tres
> cursos TERMINADO Y AUDITADO** (284 OA, 8.490 preguntas: 2.790 de 5° + 2.970 de 6° + 2.730 de 4°).
> Lo único que falta para cerrar el Bloque B del todo es lo que no es banco: fork de 4°, su voz
> (única entre los tres con voz pregrabada) y su arte propio. **5° y 6° ya están aprobados,
> cableados, con arte propio y publicados en `vulpo.cl` (06/09); 4° tiene su banco aprobado
> (06/09) pero sigue sin fork, sin voz y sin arte, así que no se anuncia todavía.**
>
> **Dos hallazgos de esta sesión que valen para cualquier banco futuro, ambos en el script
> compartido de voz:** `normalizar-voz-nivel.py` no convertía "de 1.000 en 1.000" (el punto de los
> miles cortaba la construcción antes del `en`) y tampoco un número negativo suelto como opción
> ("-32" sonaba igual que "32", porque las dos reglas de guion exigían un dígito ANTES del signo).
> Los dos se encontraron auditando (no leyendo el texto) y ya están corregidos en el script — 3° no
> los tenía porque no usa números negativos ni conteos de mil en mil.
>
...
2. **La precarga cache-first que proponen es inviable.** `assets/` son 840 MB (07/09), de los
   cuales **602 MB son la voz de 3° y 4°**: cachearlo todo en el `install` le bajaría cientos de
...
| # | Tarea | Peso | Quién |
|---|---|---|---|
| G1 | ✅ **HECHA (Sesión 130).** Lente de Formación Ciudadana en el panel: `assets/plan/oa-civica.json` (los 43 OA cívicos, generados de los 6 `oa.json` por `eje`) + informe de colegio (`kimun_prof_civica`, solo cobertura, sin acierto ni nombres — restricción en la firma) + vista por curso (dominio + fechas del plan + observaciones guardadas por curso, `civica_obs`). Fechas POR CURSO reutilizando la planificación (Sesión 108). Aplicado y verde en producción. **Cero contenido nuevo.** Evidencia del Plan de Formación Ciudadana (Ley 20.911). | medio | ✅ |
| G2 | ✅ **HECHA (Sesión 130).** `contenido/historia-8basico/oa.json` ganó su `nota_evaluacion` de nivel superior (era el único de los 6 sin ella); `HI08 OA 17/19` valorativos quedan con la advertencia de medición que 3°-7° ya tenían. Solo datos, JSON válido. | chico | ✅ |
...
- ✅ **Enlace de agenda real — HECHO (07/09/2026).** El cierre de la landing lleva
  ...
  que quita el límite de una sola programación de las cuentas gratuitas.
- ✅ **Correo propio — HECHO (07/09/2026): `contacto@vulpo.cl`**, con SPF, DKIM y DMARC
  verificados en PASS. VULPO ya no le escribe a un colegio desde un Gmail.
  **Queda en la consola, cuando haya ánimo:** cambiar el nombre visible de *"Vulpo Chile"* a
  **Roberto Lorca** —es lo que ve un director en su bandeja— y activar la **verificación en 2
  pasos**, tanto en Workspace como en **Cloudflare**, donde vive el DNS que sostiene el juego.
...
10. **Al sacar código a un módulo que encapsula (IIFE), recorrer los SEIS forks buscando quién
    llama al nombre INTERNO.** Un nombre que antes era global y ahora vive dentro del módulo
    falla **mudo**: `ReferenceError` dentro de un `onclick` no llega a la consola de forma
    fiable, así que el botón simplemente no hace nada y todo lo demás se ve bien. Pasó con
    `volverAlCapituloMate` en el ✕ del quiz: la instrucción de migrarlo estaba **escrita en la
    cabecera de `lecciones.js` desde la Sesión 83**, se cumplió solo en 8°, y 3°, 5° y 7°
    quedaron con la ✕ muerta **en producción** hasta que Roberto la usó en la Sesión 98.
    `motor.js` y `voz.js` **no** encapsulan: llamarlos a secas es lo correcto.
