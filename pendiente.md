...
| **A25** | **Mini-clases en 3° y 7°, e introducciones de Ciencias.** Salió del encargo de A24. **Alcance decidido: 45 mini-clases** (7°: 19, 3°: 26) con el mapa «enseña → desafío» de 8°, más **13 introducciones de Ciencias** (una por capítulo). ⚠️ **4°, 5° y 6° NO entran**: la práctica saca del banco (`fromBank`) y sus bancos no existen — dependencia dura con el Bloque B, sus 78 quedan planificadas. **Tres cosas que hay que saber antes de empezar:** (1) ⚠️ **ninguna herramienta lee `lecciones.json`**, así que las 17 de 8° llevan **enseñando sin firma desde la Sesión 29** y la aprobación va **antes** de escribir; (2) el motor vive **solo en 8°** y hay que extraerlo a `assets/js/lecciones.js` (479 líneas + 4 funciones sueltas por balance de llaves), y ⚠️ **`motor.js` nombra tres de ellas** —`renderCampañaMate`, `volverAlCapituloMate`, `abrirMiniClaseDeOA`—, así que cortarlas sin reconectar mata los tres cursos; (3) las introducciones de Ciencias **no piden una línea de motor**, porque `terminarLeccion()` ya marca completa una lección sin práctica. Voz de 3°: ~230 clips ≈ **US$0,15**, al final y con autorización. Plan de **18 tareas en 5 fases**: [`docs/superpowers/plans/2026-09-02-miniclases-e-introducciones.md`](docs/superpowers/plans/2026-09-02-miniclases-e-introducciones.md) | ~3-4 sesiones | código |
