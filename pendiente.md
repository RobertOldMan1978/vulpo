...
| **A56** | ⏳ **ABIERTA (de Roberto) — dos pegados chicos.** **(1) El portero de `kimun_prof_resumen`.** Medido contra producción el 08/09: devuelve **200 `[]`** donde todas las demás `kimun_prof_*` devuelven **400 `no_autorizado`**. ⚠️ **No hay fuga** —su CTE filtra por `kimun_prof_acceso`, así que un anónimo recibe la lista vacía— pero es una **excepción silenciosa a un patrón**, y `kimun_prof_listar`, que es su hermana directa y a la que esta función reemplaza en parte, sí lanza. Corregido en el repositorio; falta re-pegar `aplicar-109.sql` (v2). **No corre prisa**: lo que arregla es la consistencia del contrato, no un agujero. **(2) `seed-anio-8vo.sql`** (fuera del repo, con su `LEEME`), que fabrica **26 fotos semanales del 15/03 al 06/09/2026** en el 8° de prueba para poder mostrarle a un colegio lo que VULPO hace con un año de uso. ⚠️ **Reemplaza el desempeño de `CUR-BA04`** pero **no borra ningún perfil**, que es toda la diferencia con `seed-CUR-BA04.sql` —ese empieza con `delete from perfiles` y las fotos cuelgan con `ON DELETE CASCADE`, así que se las llevaría—. Los dos **no son intercambiables** | abierta | Roberto |
