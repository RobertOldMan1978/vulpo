...
- **Backend:** los códigos de los seis cursos en las dos listas de `kimun_prof_asignaturas`, y la
  foto semanal agendada y **tomando fotos desde el 31/08**.
  ⚠️ **`schema.sql` tiene un cambio SIN APLICAR (07/09): `kimun_prof_tendencia`**, que es la
  función que alimenta la tendencia semanal del panel. Hasta que Roberto lo re-aplique, ese
  bloque dice *"no se pudo cargar"* — **degrada, no rompe**: el mapa, la participación y el
  ranking se pintan igual (verificado quitándole la función al cliente).
...
| **A49** | ⏳ **ABIERTA — las ideas del panel del profesor que Roberto no eligió (07/09, Sesión 106).** Salieron de mirar el panel con un doble de Supabase; quedaron sin hacer a propósito, no por olvido. Por orden de lo que cambia: **(a) La más grande y la que hay que decidir primero: bajar el registro visual.** Titan One, violeta saturado, fondo de cosmos y botones de 20 px de radio en una herramienta que mira una UTP con un presupuesto en la mano. Es la misma tensión que la landing resolvió en la Sesión 51 pasando a Inter; **conviene decidirla ANTES de tocar más CSS del panel, porque define todo lo demás**. **(b) Una franja de "qué necesita tu atención" al entrar** —*7 alumnos no han entrado esta semana · 3 objetivos bajo 70% · 1 refuerzo sin cerrar*—, cada línea enlazando a su acción: hoy el panel abre en una lista de cursos plegados y no dice qué hacer. Es la diferencia entre una base de datos y una herramienta. **(c) La participación como titular de la tarjeta del curso**: *"15/22 jugaron esta semana"* es la línea más valiosa de esa tarjeta y va en cian pequeño, igual que "Profesor jefe", **y no dice si 15 de 22 está bien o mal**. **(d) Exportar o imprimir el mapa** para una reunión de departamento: hoy no hay forma de sacarlo de la pantalla. ⚠️ Y una advertencia que vale para las cuatro: **el panel está en producción y es la pantalla que se muestra para vender.** Los cambios visuales son seguros; los de flujo piden el cuidado de siempre, porque hay cuatro roles y lo que un profe de asignatura no debe ver está resuelto en el servidor *y* en la interfaz | abierta | código |
